...
begin
	...
	puts ("[+] #{site} is running Joomla #{version[1]}")
	exit
...
begin
    ...
    version = page.body.match("Version.([0-9]\.*[0-9].*)")
    puts ("[+] #{site} is running Wordpress #{version[1]}")
    exit
    rescue Mechanize::ResponseCodeError
	rescue NoMethodError
end
...
begin
    ...
    version = page.body.match("version.=.([0-9]\.*[0-9].*)")
    puts ("[+] #{site} is running phpBB #{version[1]}")
    exit
    rescue Mechanize::ResponseCodeError
	rescue NoMethodError
end
...
begin
    ...
    puts ("[+] #{site} is running Drupal #{version[1]}")
    exit
    rescue Mechanize::ResponseCodeError
	rescue NoMethodError
end
...
begin
    ...
    puts ("[+] #{site} is running SMF #{version[1]}")
	exit
    rescue Mechanize::ResponseCodeError
	rescue NoMethodError
end
